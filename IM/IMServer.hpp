...
class MysqlClient{
  private:
    ...
    bool ConnectMysql()
    {
      ...
      if(!mysql_real_connect(my, "localhost", "root", "1123102244", IM_DB, MY_PORT, NULL, 0))
      {
...
class IMServer{
  ...
  public:
    ...
    static void LoginHandler(mg_connection *nc, int ev, void *data)
    {
      ...
      transform(method.begin(), method.end(), method.begin(), ::toupper);/* a->A */
      if (method == "POST")
      {
      ...
      //cout << "call Login handler : " << ev << "url : " << uri << endl;
      //mg_serve_http(nc, hm, s_http_server_opts);
    }
